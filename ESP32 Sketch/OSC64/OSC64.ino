...
// Declare oscServerIP and oscServerPort as global variables
String oscServerIP = "192.168.178.244";
String oscServerPort = "8888";
String oscServerIP2 = "192.168.178.244";
String oscServerPort2 = "8889";
String oscServerIP3 = "192.168.178.244";
String oscServerPort3 = "8890";
String oscServerIP4 = "192.168.178.244";
String oscServerPort4 = "8891";
String oscServerIP5 = "192.168.178.244";
String oscServerPort5 = "8892";
...
void setup() {
  ...
  regID = settings.getString("regID", "bdc40bbe973edd37");

  // get the nick name from the eeprom
  myNickName = settings.getString("myNickName", "IDLab");
...
void loop() {
  ...
  if (dataFromC64) {
      ...
      case 252:
        {
          ...
          char wifiData[256];
          for (int i = 0; i < inbuffersize && i < 255; i++) {
              wifiData[i] = screenCode_to_Ascii(inbuffer[i]);
          }
          wifiData[inbuffersize] = '\0';

          // Split into fields
          String data = String(wifiData);
          int firstDelim = data.indexOf(char(129));
          int secondDelim = data.indexOf(char(129), firstDelim + 1);

          if (firstDelim != -1 && secondDelim != -1) {
              ssid = data.substring(0, firstDelim);
              password = data.substring(firstDelim + 1, secondDelim);
              timeoffset = data.substring(secondDelim + 1);
              ...
              Serial.printf("WiFi Setup: SSID='%s', Pass='%s', Time='%s'\n", 
                           ssid.c_str(), password.c_str(), timeoffset.c_str());
          } else {
              Serial.println("Error: Invalid WiFi setup format");
          }
          break;
        }
    ...
    {
        receive_buffer_from_C64(1);
        char combinedData[256];
        for (int i = 0; i < inbuffersize && i < 255; i++) {
            combinedData[i] = screenCode_to_Ascii(inbuffer[i]);
        }
        combinedData[inbuffersize] = '\0';

        // Debug raw input
        Serial.printf("Raw combined data: ");
        for (int j = 0; j < inbuffersize; j++) Serial.printf("%02X ", inbuffer[j]);
        Serial.println();

        // Split into regID and nickname
        String data = String(combinedData);
        int splitPos = data.indexOf(char(129));
        
        if (splitPos != -1) {
            regID = data.substring(0, splitPos);
            regID.trim();
            myNickName = data.substring(splitPos + 1);
            myNickName.trim();
            ...
            Serial.printf("Saved: regID='%s', nickname='%s'\n", regID.c_str(), myNickName.c_str());
        } else {
            Serial.println("Error: No delimiter in data. Data format: regID + char(129) + nickname");
            sendByte(255); // Error code
        }

        sendByte(128); // Acknowledge
        break;
    }
      case 239:
        {
      ...
      case 230:
        {
          ...
          Serial.println(ii);
          if (ii == 0 or ii == 1) {
            adminMessageIndex += ii;
            gotAdminMessage = 0;
            commandMessage.command = GetAdminMessage;
            xMessageBufferSend(commandBuffer, &commandMessage, sizeof(commandMessage), portMAX_DELAY);

            while (gotAdminMessage == 0) {
              delay(500);
              Serial.println("wait for message");
            }
            Serial.write(adminMessage, strlen(adminMessage));
            Serial.println();
      ...
      case 229:
        {
          receive_buffer_from_C64(2); // Get IP + port pairs
          ...
          for (i = 0; i < inbuffersize && i < 255; i++) {
              ...
              if (oscMsg[i] == '\0') break; // Stop at null terminator
          }
          oscMsg[i] = '\0'; // Ensure null-termination

          // Debug: Log raw and converted data
          Serial.printf("Raw C64 input (hex): ");
          for (int j = 0; j < inbuffersize; j++) Serial.printf("%02X ", inbuffer[j]);
          Serial.println();
          Serial.printf("Converted input: %s\n", oscMsg);

          // Split into 5 IP/port pairs (delimited by char(130))
          char* pair = strtok(oscMsg, "\x82"); // 130 in hex
          int pairIndex = 0;

          while (pair != nullptr && pairIndex < 5) {
              // Split IP and port (delimited by char(129))
              char* delimiter = strchr(pair, '\x81'); // 129 in hex
              if (delimiter) {
                  *delimiter = '\0'; // Split into IP and port
                  String ip = String(pair);
                  String port = String(delimiter + 1);

                  // Trim whitespace/garbage from port
                  port.trim();
                  ...
                  Serial.printf("Pair %d: IP='%s', Port='%s'\n", pairIndex, ip.c_str(), port.c_str());
              }
              pair = strtok(nullptr, "\x82"); // Move to next pair
              pairIndex++;
          }
          ...
          Serial.println("EEPROM save completed for all OSC pairs");
          break;
        }
  ...
  case 227:
        {
          ...
          // ------------------------------------------------------------------------------
          commandMessage.command = GetRegistrationStatusCommand;
          Serial.printf("Sending to buffer: %p\n", commandBuffer);
          if (commandBuffer == NULL) {
            Serial.println("ERROR: commandBuffer is NULL!");
            return;
          }
          xMessageBufferSend(commandBuffer, &commandMessage, sizeof(commandMessage), portMAX_DELAY);
          xMessageBufferReceive(responseBuffer, &responseMessage, sizeof(responseMessage), portMAX_DELAY);
                    // Combine all IPs and ports into a single string
          String response = 
              oscServerIP + char(129) + oscServerPort + char(130) +
              oscServerIP2 + char(129) + oscServerPort2 + char(130) +
              oscServerIP3 + char(129) + oscServerPort3 + char(130) +
              oscServerIP4 + char(129) + oscServerPort4 + char(130) +
              oscServerIP5 + char(129) + oscServerPort5;

          send_String_to_c64(response);
          Serial.print(oscServerIP);
          Serial.print(":");
          Serial.println(oscServerPort);
          Serial.print(oscServerIP2);
          Serial.print(":");
          Serial.println(oscServerPort2);
          Serial.print(oscServerIP3);
          Serial.print(":");
          Serial.println(oscServerPort3);
          Serial.print(oscServerIP4);
          Serial.print(":");
          Serial.println(oscServerPort4);
          Serial.print(oscServerIP5);
          Serial.print(":");
          Serial.println(oscServerPort5);
          break;
        }
          // ------------------------------------------------------------------------------
          // start byte 226 = C64 sends the new osc message to the ESP32
          // ------------------------------------------------------------------------------        
      case 226:  // Handle OSC messages
      {
          if (!isWifiCoreConnected || !udpInitialized) {
              sendByte(128);
              break;
          }
          ...
          if (!addressEnd) {
              // No arguments - send simple message
              for (int j = 0; j < 5; j++) {
                  if (ipPortPairs[j][0] != "0.0.0.0" && ipPortPairs[j][1] != "0") {
                      OSCMessage msg(oscMsg);
                      Udp.beginPacket(ipPortPairs[j][0].c_str(), ipPortPairs[j][1].toInt());
                      msg.send(Udp);
                      Udp.endPacket();
                  }
              }
...
bool receive_buffer_from_C64(int cnt) {
  ...
  while (cnt > 0) {
    ...
    dataFromC64 = false;
    inbuffer[i] = ch;
    sendByte(128);  // <-- ACKNOWLEDGE RECEIPT
    i++;
...
void loadPrgfile() {
  ...
  for (int x = 2; x < sizeof(prgfile); x++) {  // Now send all the rest of the bytes
    if (sendByte(prgfile[x])) {
      Serial.println("Loading failed, Reboot!");
      delay(100);
      reboot();
      break;
    };
...
void sendOSCMessage(const char* address, const char* message) {
  ...
  msg.add(message);
  Udp.beginPacket(oscServerIP.c_str(), oscServerPort.toInt());
  msg.send(Udp);
  Udp.endPacket();
}
