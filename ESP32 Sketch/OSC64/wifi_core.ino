...
// Define the global udpInitialized variable
...
void get_admin_message() {
  ...
  String httpRequestData = "regid=" + regID + "&line=1&last=" + lastSysMessage + "&index=" + adminMessageIndex;
...
void WifiCoreLoop(void* parameter) {
  ...
  for (;;) {  // this is an endless loop
    ...
    if (isWifiCoreConnected && !udpInitialized) {
      // Initialize UDP only once after WiFi is connected
      if (Udp.begin(8888)) {
